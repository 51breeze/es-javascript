...
class IfStatement extends Syntax{
    emiter(syntax){
        ...
        if(this.stack.hasAwait){
            ...
            if( !alternate ){
                if( !aRet ){
                    expression.push(`${topIndent}\t${stack.generatorVarName("_a",true)}.label=${labelIndex};`);
                }
                expression.push(`${topIndent}case ${labelIndex}:`);
                return expression.join("\r\n");
            }else{
                if( !aRet ){
                    const nextLabel = ++stack.awaitCount;
                    expression.push(`${topIndent}\treturn [3,${nextLabel}];`);
                }
                expression.push(`${topIndent}case ${labelIndex}:`);
                expression.push(alternate);
                if( !isReturnNode(this.stack.alternate) ){
                    expression.push(`${topIndent}\t${stack.generatorVarName("_a",true)}.label=${nextLabel};`);
                }
                if( !aRet ){
