...
function getProperties(classObject){
    ...
    const descriptor = Reflect.getDescriptors(classObject, {mode:2})
    if(descriptor){
        descriptor.descriptors.forEach(desc=>{
            if(desc.isStatic && desc.permission ==='public' && desc.label === 'enum-property'){
