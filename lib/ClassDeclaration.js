...
class ClassDeclaration extends Syntax{
    ...
    buildExternal(){
        ...
        if( stack && stack.externals.length > 0 ){
            const externals = stack.externals.map( item=>item.emiter(this, this.module) ).filter(item=>!!item);
    ...
    emiter( syntax ){
        ...
        const refs = [];
        const emiter=(target,proto,content,isStatic,descriptive)=>{
        ...
        emiter( methods , 'methods', methodContent , true);
        emiter( members , `members`, memberContent , false);
        ...
        const construct = module.methodConstructor ? module.methodConstructor.emiter( syntax ) : `${defaultConstructor.join("\r\n")}`;
        const description = [
