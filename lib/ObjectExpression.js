...
class ObjectExpression extends Syntax{
    ...
    objectExpression(properties){
        ...
        if( this.stack.hasChildComputed  ){
            ...
            const refs = this.stack.generateVarName("_c");
            if( statementStack ){
    ...
    emiter(syntax){
        ...
        if( spreadElementIndex >=0 ){
            ...
            do{
                ...
                {
                    props.push( this.objectExpression( left.map( item=>item.emiter(syntax) ) ) );
                }
            ...
            {
                props.push( this.objectExpression( properties.map( item=>item.emiter(syntax) ) ) );
            }
            return this.objectMerge(props);
        }else{
            return this.objectExpression( properties.map( item=>item.emiter(syntax) ) );
        }
    }
}
